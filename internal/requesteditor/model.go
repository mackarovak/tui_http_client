...
func (m Model) renderMethodSelector() string {
	...
	for i, method := range types.HTTPMethods {
		...
		if i == m.methodIdx && m.focusField == FieldMethod {
			rendered = ui.Theme.Selection.Render(rendered)
		}
		parts = append(parts, rendered)
	}
	return m.fieldPrefix(m.focusField == FieldMethod) + strings.Join(parts, " ")
}

func (m Model) renderURLField() string {
	labelStyle := ui.Theme.Muted
	if m.focusField == FieldURL {
		labelStyle = ui.Theme.FocusLabel
	}
	return labelStyle.Render(m.fieldPrefix(m.focusField == FieldURL)+"URL: ") + m.urlInput.View()
}
...
func (m Model) renderTabBar() string {
	...
	return m.fieldPrefix(m.focusField == FieldTabBar) + strings.Join(parts, "  ")
}
...
func (m Model) renderBodyTab() string {
	var modes []string
	contentFocused := m.focusField == FieldTabContent && m.activeTab == TabBody
	for i, label := range bodyModeLabels {
		if i == m.bodyModeIdx {
			rendered := ui.Theme.TabActive.Render("[" + label + "]")
			if contentFocused {
				rendered = ui.Theme.Selection.Render(rendered)
			}
			modes = append(modes, rendered)
		} else {
			modes = append(modes, ui.Theme.TabInactive.Render("["+label+"]"))
		}
	}
	modeBar := ui.Theme.Muted.Render(m.fieldPrefix(contentFocused)+"Mode: ") + strings.Join(modes, " ") + "\n"
...
func (m Model) renderAuthTab() string {
	...
	var parts []string
	contentFocused := m.focusField == FieldTabContent && m.activeTab == TabAuth
	for i, name := range authTypes {
		if i == m.authTypeIdx {
			rendered := ui.Theme.TabActive.Render("[" + name + "]")
			if contentFocused {
				rendered = ui.Theme.Selection.Render(rendered)
			}
			parts = append(parts, rendered)
	...
	header := ui.Theme.Muted.Render(m.fieldPrefix(contentFocused)+"Auth: ") + strings.Join(parts, " ") + "\n\n"
...
func (m Model) fieldPrefix(active bool) string {
	if active {
		return ui.Theme.FocusLabel.Render("> ")
	}
	return "  "
}
...
func (m *Model) applySize(w, h int) {
	...
	m.bodyInput.SetWidth(max(w-4, 1))
	m.bodyInput.SetHeight(bodyHeight)

	m.headerTable = m.headerTable.SetWidth(max(w-4, 1))
	m.paramTable = m.paramTable.SetWidth(max(w-4, 1))
	m.tokenInput.Width = max(w-20, 1)
}
